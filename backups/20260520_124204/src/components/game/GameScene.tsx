...
import { Position, TerrainType, Unit, Faction, DamagePopup, MapCell, WeatherType } from '@/game/types';
...
export default function GameScene() {
 ...
 useEffect(() => {
  ...
  const createUnitMesh = useCallback((unit: Unit): THREE.Group => {
    ...
    const baseColor = isRed ? 0xc62828 : 0x1565c0;
    const darkColor = isRed ? 0x8e0000 : 0x0d47a1;

    const baseGeo = new THREE.CylinderGeometry(0.35, 0.38, 0.05, 16);
    ...
    // v78.0: Ground shadow decal beneath each unit for visual grounding
    ...
    shadowDecal.renderOrder = -1; // Render before everything else
    shadowDecal.name = 'groundShadow';
    // v78.0: Hero units get a larger, faction-tinted shadow
    if (unit.isHero) {
      shadowDecal.scale.setScalar(1.3);
      shadowMat.color.setHex(isRed ? 0xc62828 : 0x1565c0);
      shadowMat.opacity = 0.15;
    }
    group.add(shadowDecal);

    // [REMOVED] Health status ring per user request

    switch (unit.type) {
      case 'tank': {
        const bodyGeo = new THREE.BoxGeometry(0.5, 0.15, 0.35);
        const bodyMat = new THREE.MeshStandardMaterial({ color: darkColor, metalness: 0.6, roughness: 0.4 });
        const body = new THREE.Mesh(bodyGeo, bodyMat);
        body.position.y = 0.12;
        group.add(body);

        const turretGeo = new THREE.CylinderGeometry(0.14, 0.16, 0.12, 8);
        const turretMat = new THREE.MeshStandardMaterial({ color: baseColor, metalness: 0.6, roughness: 0.3 });
        const turret = new THREE.Mesh(turretGeo, turretMat);
        turret.position.y = 0.24;
        turret.name = 'turret';
        group.add(turret);

        const barrelGeo = new THREE.CylinderGeometry(0.03, 0.03, 0.35, 6);
        const barrelMat = new THREE.MeshStandardMaterial({ color: 0x424242, metalness: 0.8 });
        const barrel = new THREE.Mesh(barrelGeo, barrelMat);
        barrel.rotation.z = Math.PI / 2;
        barrel.position.set(0.2, 0.24, 0);
        group.add(barrel);

        for (const zOffset of [-0.2, 0.2]) {
          const trackGeo = new THREE.BoxGeometry(0.55, 0.08, 0.08);
          const trackMat = new THREE.MeshStandardMaterial({ color: 0x212121, roughness: 0.9 });
          const track = new THREE.Mesh(trackGeo, trackMat);
          track.position.set(0, 0.08, zOffset);
          group.add(track);
        }
        break;
      }
      case 'ifv': {
        const bodyGeo = new THREE.BoxGeometry(0.45, 0.12, 0.3);
        const bodyMat = new THREE.MeshStandardMaterial({ color: darkColor, metalness: 0.5, roughness: 0.4 });
        const body = new THREE.Mesh(bodyGeo, bodyMat);
        body.position.y = 0.1;
        group.add(body);

        const turretGeo = new THREE.CylinderGeometry(0.1, 0.12, 0.1, 8);
        const turretMat = new THREE.MeshStandardMaterial({ color: baseColor, metalness: 0.5 });
        const turret = new THREE.Mesh(turretGeo, turretMat);
        turret.position.y = 0.2;
        turret.name = 'turret';
        group.add(turret);

        const gunGeo = new THREE.CylinderGeometry(0.02, 0.02, 0.25, 6);
        const gunMat = new THREE.MeshStandardMaterial({ color: 0x424242, metalness: 0.8 });
        const gun = new THREE.Mesh(gunGeo, gunMat);
        gun.rotation.z = Math.PI / 2;
        gun.position.set(0.17, 0.2, 0);
        group.add(gun);

        for (const zOffset of [-0.18, 0.18]) {
          const trackGeo = new THREE.BoxGeometry(0.48, 0.06, 0.06);
          const trackMat = new THREE.MeshStandardMaterial({ color: 0x212121, roughness: 0.9 });
          const track = new THREE.Mesh(trackGeo, trackMat);
          track.position.set(0, 0.06, zOffset);
          group.add(track);
        }
        break;
      }
      case 'artillery': {
        ...
        const bodyMat = new THREE.MeshStandardMaterial({ color: darkColor, metalness: 0.4, roughness: 0.5 });
        const body = new THREE.Mesh(bodyGeo, bodyMat);
        body.position.y = 0.1;
        group.add(body);

        const barrelGeo = new THREE.CylinderGeometry(0.04, 0.035, 0.45, 6);
        const barrelMat = new THREE.MeshStandardMaterial({ color: 0x616161, metalness: 0.7 });
        const barrel = new THREE.Mesh(barrelGeo, barrelMat);
        barrel.rotation.z = Math.PI / 4;
        barrel.position.set(0.15, 0.3, 0);
        barrel.name = 'barrel';
        group.add(barrel);

        const supportGeo = new THREE.BoxGeometry(0.08, 0.15, 0.3);
        const supportMat = new THREE.MeshStandardMaterial({ color: baseColor, metalness: 0.4 });
        const support = new THREE.Mesh(supportGeo, supportMat);
        support.position.set(-0.12, 0.15, 0);
        group.add(support);
        break;
      }
      case 'scout': {
        const bodyGeo = new THREE.BoxGeometry(0.35, 0.08, 0.22);
        const bodyMat = new THREE.MeshStandardMaterial({ color: darkColor, metalness: 0.5, roughness: 0.3 });
        const body = new THREE.Mesh(bodyGeo, bodyMat);
        body.position.y = 0.09;
        group.add(body);

        const antennaGeo = new THREE.CylinderGeometry(0.01, 0.01, 0.3, 4);
        const antennaMat = new THREE.MeshStandardMaterial({ color: 0xeeeeee });
        const antenna = new THREE.Mesh(antennaGeo, antennaMat);
        antenna.position.set(-0.05, 0.25, 0);
        antenna.name = 'antenna';
        group.add(antenna);

        const sensorGeo = new THREE.SphereGeometry(0.04, 8, 6);
        const sensorMat = new THREE.MeshStandardMaterial({
          color: isRed ? 0xff5252 : 0x448aff,
          emissive: isRed ? 0xff5252 : 0x448aff,
          emissiveIntensity: 0.5
        });
        const sensor = new THREE.Mesh(sensorGeo, sensorMat);
        sensor.position.set(-0.05, 0.4, 0);
        group.add(sensor);
        ...
        for (const zOffset of [-0.13, 0.13]) {
          for (const xOffset of [-0.1, 0.1]) {
            ...
            wheel.position.set(xOffset, 0.04, zOffset);
            group.add(wheel);
          }
        }
        break;
      ...
      case 'infantry': {
        const torsoGeo = new THREE.BoxGeometry(0.15, 0.2, 0.1);
        const torsoMat = new THREE.MeshStandardMaterial({ color: darkColor, roughness: 0.7 });
        ...
        const headMat = new THREE.MeshStandardMaterial({ color: 0xffcc80, roughness: 0.8 });
        const head = new THREE.Mesh(headGeo, headMat);
        head.position.y = 0.36;
        group.add(head);

        const helmetGeo = new THREE.SphereGeometry(0.07, 8, 4, 0, Math.PI * 2, 0, Math.PI / 2);
        const helmetMat = new THREE.MeshStandardMaterial({ color: baseColor, roughness: 0.6 });
        const helmet = new THREE.Mesh(helmetGeo, helmetMat);
        helmet.position.y = 0.37;
        group.add(helmet);

        const rifleGeo = new THREE.BoxGeometry(0.03, 0.03, 0.25);
        const rifleMat = new THREE.MeshStandardMaterial({ color: 0x3e2723, roughness: 0.9 });
        const rifle = new THREE.Mesh(rifleGeo, rifleMat);
        rifle.position.set(0.1, 0.22, 0);
        group.add(rifle);

        for (const zOffset of [-0.04, 0.04]) {
          ...
          leg.position.set(0, 0.08, zOffset);
          group.add(leg);
        }
        break;
      }
      case 'sam': {
        ...
        // Rotating radar dish - cylinder base
        const radarBaseGeo = new THREE.CylinderGeometry(0.04, 0.06, 0.15, 8);
        const radarBaseMat = new THREE.MeshStandardMaterial({ color: 0x616161, metalness: 0.7 });
        const radarBase = new THREE.Mesh(radarBaseGeo, radarBaseMat);
        radarBase.position.y = 0.22;
        group.add(radarBase);

        // Radar disc
        const radarDiscGeo = new THREE.CylinderGeometry(0.18, 0.18, 0.02, 16);
        const radarDiscMat = new THREE.MeshStandardMaterial({
          color: isRed ? 0xff5252 : 0x448aff,
          emissive: isRed ? 0xff5252 : 0x448aff,
        ...
        radarDisc.position.y = 0.32;
        radarDisc.name = 'radar';
        radarDisc.rotation.z = Math.PI / 6;
        group.add(radarDisc);

        // Missile launcher box on back
        const launcherGeo = new THREE.BoxGeometry(0.15, 0.12, 0.2);
        const launcherMat = new THREE.MeshStandardMaterial({ color: baseColor, metalness: 0.5 });
        const launcher = new THREE.Mesh(launcherGeo, launcherMat);
        launcher.position.set(-0.1, 0.2, 0);
        launcher.rotation.z = Math.PI / 8;
        group.add(launcher);

        for (const zOffset of [-0.18, 0.18]) {
          const trackGeo = new THREE.BoxGeometry(0.44, 0.06, 0.06);
          const trackMat = new THREE.MeshStandardMaterial({ color: 0x212121, roughness: 0.9 });
          const track = new THREE.Mesh(trackGeo, trackMat);
          track.position.set(0, 0.06, zOffset);
          group.add(track);
      ...
      case 'engineer': {
        ...
        // Crane arm - angled cylinder
        const craneGeo = new THREE.CylinderGeometry(0.025, 0.025, 0.4, 6);
        const craneMat = new THREE.MeshStandardMaterial({ color: 0xffa000, metalness: 0.6 });
        const crane = new THREE.Mesh(craneGeo, craneMat);
        crane.rotation.z = Math.PI / 4;
        crane.position.set(0.1, 0.3, 0.1);
        group.add(crane);

        // Crane hook
        const hookGeo = new THREE.SphereGeometry(0.03, 6, 4);
        const hookMat = new THREE.MeshStandardMaterial({ color: 0x616161, metalness: 0.8 });
        const hook = new THREE.Mesh(hookGeo, hookMat);
        hook.position.set(0.25, 0.38, 0.1);
        group.add(hook);

        // Dozer blade in front
        const bladeGeo = new THREE.BoxGeometry(0.06, 0.12, 0.35);
        const bladeMat = new THREE.MeshStandardMaterial({ color: 0xffa000, metalness: 0.6, roughness: 0.4 });
        const blade = new THREE.Mesh(bladeGeo, bladeMat);
        blade.position.set(0.28, 0.08, 0);
        group.add(blade);
      ...
      case 'supply': {
        // Larger truck-like body
        const cabGeo = new THREE.BoxGeometry(0.2, 0.12, 0.28);
        const cabMat = new THREE.MeshStandardMaterial({ color: darkColor, metalness: 0.4, roughness: 0.5 });
        const cab = new THREE.Mesh(cabGeo, cabMat);
        cab.position.set(0.12, 0.1, 0);
        group.add(cab);

        // Cargo cover on back
        const cargoGeo = new THREE.BoxGeometry(0.28, 0.14, 0.26);
        const cargoMat = new THREE.MeshStandardMaterial({ color: baseColor, metalness: 0.3, roughness: 0.6 });
        const cargo = new THREE.Mesh(cargoGeo, cargoMat);
        cargo.position.set(-0.1, 0.12, 0);
        group.add(cargo);

        // Cargo tarp ridge
        const tarpGeo = new THREE.BoxGeometry(0.28, 0.02, 0.26);
        const tarpMat = new THREE.MeshStandardMaterial({ color: isRed ? 0x8e0000 : 0x0d47a1, roughness: 0.8 });
        const tarp = new THREE.Mesh(tarpGeo, tarpMat);
        tarp.position.set(-0.1, 0.2, 0);
        group.add(tarp);

        // Cross symbol on side
        const crossVGeo = new THREE.BoxGeometry(0.02, 0.08, 0.04);
        const crossHGeo = new THREE.BoxGeometry(0.02, 0.04, 0.08);
        const crossMat = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xffffff, emissiveIntensity: 0.3 });
        const crossV = new THREE.Mesh(crossVGeo, crossMat);
        crossV.position.set(0.04, 0.12, 0.15);
        crossV.name = 'crossV';
        group.add(crossV);
        const crossH = new THREE.Mesh(crossHGeo, crossMat);
        crossH.position.set(0.04, 0.12, 0.15);
        crossH.name = 'crossH';
        group.add(crossH);

        for (const zOffset of [-0.16, 0.16]) {
          for (const xOffset of [-0.12, 0, 0.12]) {
            const wheelGeo = new THREE.CylinderGeometry(0.04, 0.04, 0.03, 8);
            const wheelMat = new THREE.MeshStandardMaterial({ color: 0x212121 });
            const wheel = new THREE.Mesh(wheelGeo, wheelMat);
            wheel.rotation.x = Math.PI / 2;
      ...
      case 'helicopter': {
        ...
        // Cockpit (front, slightly raised)
        const cockpitGeo = new THREE.SphereGeometry(0.08, 8, 6);
        const cockpitMat = new THREE.MeshStandardMaterial({
          color: isRed ? 0xff8a80 : 0x82b1ff,
          transparent: true,
          opacity: 0.7,
          metalness: 0.8,
          roughness: 0.1,
        ...
        cockpit.position.set(0.18, 0.38, 0);
        cockpit.scale.set(1, 0.6, 0.8);
        group.add(cockpit);

        // Main rotor disc (flat cylinder on top)
        const rotorGeo = new THREE.CylinderGeometry(0.3, 0.3, 0.01, 16);
        ...
        const hubMat = new THREE.MeshStandardMaterial({ color: 0x424242, metalness: 0.7 });
        const hub = new THREE.Mesh(hubGeo, hubMat);
        hub.position.y = 0.42;
        group.add(hub);

        // Tail boom
        const tailGeo = new THREE.BoxGeometry(0.25, 0.04, 0.06);
        const tailMat = new THREE.MeshStandardMaterial({ color: darkColor, metalness: 0.4 });
        const tail = new THREE.Mesh(tailGeo, tailMat);
        tail.position.set(-0.25, 0.38, 0);
        group.add(tail);

        // Tail rotor (small vertical disc)
        const tailRotorGeo = new THREE.CylinderGeometry(0.06, 0.06, 0.005, 8);
        ...
        tailRotor.position.set(-0.37, 0.42, 0);
        ...
        for (const zOffset of [-0.1, 0.1]) {
          const podGeo = new THREE.BoxGeometry(0.2, 0.04, 0.04);
          const podMat = new THREE.MeshStandardMaterial({ color: 0x424242, metalness: 0.6 });
          const pod = new THREE.Mesh(podGeo, podMat);
          pod.position.set(0, 0.32, zOffset);
          group.add(pod);
        }

        // Landing skids
        for (const zOffset of [-0.08, 0.08]) {
          const skidGeo = new THREE.BoxGeometry(0.25, 0.01, 0.01);
          const skidMat = new THREE.MeshStandardMaterial({ color: 0x424242, metalness: 0.6 });
          const skid = new THREE.Mesh(skidGeo, skidMat);
          skid.position.set(0, 0.26, zOffset);
          group.add(skid);
        }
        break;
      }
      case 'mlrs': {
        // Box body
        const bodyGeo = new THREE.BoxGeometry(0.45, 0.1, 0.3);
        const bodyMat = new THREE.MeshStandardMaterial({ color: darkColor, metalness: 0.5, roughness: 0.4 });
        const body = new THREE.Mesh(bodyGeo, bodyMat);
        body.position.y = 0.1;
        group.add(body);

        // Elevated rocket pod (angled box on back)
        const podGeo = new THREE.BoxGeometry(0.2, 0.12, 0.18);
        const podMat = new THREE.MeshStandardMaterial({ color: baseColor, metalness: 0.5 });
        const pod = new THREE.Mesh(podGeo, podMat);
        pod.position.set(-0.08, 0.22, 0);
        pod.name = 'pod';
        pod.rotation.z = Math.PI / 6; // angled upward
        group.add(pod);

        // Rocket tubes (small cylinders in the pod)
        for (let row = 0; row < 2; row++) {
          for (let col = 0; col < 3; col++) {
            const tubeGeo = new THREE.CylinderGeometry(0.02, 0.02, 0.2, 6);
            const tubeMat = new THREE.MeshStandardMaterial({ color: 0x424242, metalness: 0.8 });
            const tube = new THREE.Mesh(tubeGeo, tubeMat);
            tube.name = 'tube';
            const zOff = (col - 1) * 0.045;
            const yOff = row * 0.04 - 0.02;
            tube.rotation.z = Math.PI / 6;
            tube.position.set(-0.08 + 0.06, 0.22 + yOff, zOff);
            group.add(tube);
          }
        }

        // Support struts
        const strutGeo = new THREE.BoxGeometry(0.03, 0.1, 0.03);
        const strutMat = new THREE.MeshStandardMaterial({ color: 0x616161, metalness: 0.6 });
        const strut1 = new THREE.Mesh(strutGeo, strutMat);
        strut1.position.set(-0.02, 0.17, 0.06);
        group.add(strut1);
        const strut2 = new THREE.Mesh(strutGeo, strutMat);
        strut2.position.set(-0.02, 0.17, -0.06);
        group.add(strut2);
    ...
    // v90.0: Enable shadow casting on all unit body meshes (MeshStandardMaterial only)
    ...
    // [REMOVED] HP Bar Sprite per user request

    // Armor indicator bar (amber, below HP bar) - only show for units with armor > 5
    if (unit.stats.armor > 5) {
      const armorPercent = Math.min(1, unit.stats.armor / 40); // 40 = max armor
      ...
      armorBg.position.y = 0.48;
      armorBg.rotation.x = -Math.PI / 4;
      // Insert before the armor bar so it's behind
      group.add(armorBg);
      // Re-add armor bar on top
      group.add(armorBar);
    }

    // Type icon ring
    const iconGeo = new THREE.RingGeometry(0.12, 0.14, 16);
    ...
    // v82.0: Unit type badge dot — small colored sphere on top of unit for quick visual identification
    const UNIT_TYPE_DOT_COLORS: Record<string, number> = {
      tank: 0xff4444,
      ifv: 0xff8800,
      artillery: 0xffcc00,
      scout: 0x44ff44,
    ...
    const dotColor = UNIT_TYPE_DOT_COLORS[unit.type] ?? 0xffffff;
    const dotGeo = new THREE.SphereGeometry(0.04, 6, 4);
    const dotMat = new THREE.MeshBasicMaterial({
      color: dotColor,
      transparent: true,
      opacity: 0.9,
    });
    const typeDot = new THREE.Mesh(dotGeo, dotMat);
    typeDot.name = 'typeDot';
    typeDot.position.y = 0.6; // Above the unit body
    group.add(typeDot);
